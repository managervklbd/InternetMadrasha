...
export default async function InvoicePrintPage({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-4 md:p-8">
            ...
            {/* The Invoice Card */}
            <div className="max-w-4xl mx-auto bg-white dark:bg-zinc-900 shadow-xl rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 print:shadow-none print:border-none print:m-0">
                {/* Header */}
                <div className="bg-gradient-to-r from-teal-900 to-emerald-800 p-8 text-white relative">
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-extrabold tracking-tight mb-1">{settings?.madrasaName || "ইন্টারনেট মাদ্রাসা"}</h1>
                                <p className="text-teal-100 font-medium">Official Invoice</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <h2 className="text-xl font-bold opacity-80 uppercase tracking-widest">Invoice</h2>
                ...
                <div className="p-8 space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <section className="space-y-4">
                            ...
                            <div className="space-y-1">
                                <p className="text-xl font-bold text-zinc-800 dark:text-zinc-100">{student.fullName}</p>
                        ...
                        <section className="space-y-4 md:text-right">
                            ...
                            <div className="space-y-1">
                                <p className="text-zinc-600 dark:text-zinc-400"><strong>Issued:</strong> {new Date(invoice.issuedAt).toLocaleDateString()}</p>
                                <p className="text-zinc-600 dark:text-zinc-400"><strong>Month:</strong> {invoice.month}/{invoice.year}</p>
                                <div className="pt-2">
                                    <span className={`px-4 py-1.5 rounded-full text-sm font-bold border ${invoice.status === 'PAID'
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-6 border-y border-zinc-100 dark:border-zinc-800">
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase">Course</p>
                            <p className="font-semibold text-zinc-700 dark:text-zinc-300">{student.department?.course?.name || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase">Department</p>
                            <p className="font-semibold text-zinc-700 dark:text-zinc-300">{student.department?.name || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase">Batch/Semester</p>
                            <p className="font-semibold text-zinc-700 dark:text-zinc-300">{enrollment?.batch?.name || "N/A"}</p>
                        </div>
                    </div>

                    {/* Items Table */}
                    <section>
                        <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-4">Fee Summary</h3>
                        <table className="w-full">
                            <thead className="border-b border-zinc-100 dark:border-zinc-800">
                                <tr>
                                    <th className="text-left py-3 text-sm font-bold text-zinc-600 dark:text-zinc-400">Description</th>
                                    <th className="text-right py-3 text-sm font-bold text-zinc-600 dark:text-zinc-400">Amount</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-900">
                                <tr>
                                    <td className="py-4 text-zinc-700 dark:text-zinc-300 font-medium">Monthly Tuition Fee ({invoice.month}/{invoice.year})</td>
                            ...
                            <tfoot>
                                <tr className="border-t-2 border-zinc-900 dark:border-zinc-100">
                                    <td className="py-6 text-xl font-extrabold text-zinc-900 dark:text-white uppercase tracking-tighter text-right pr-4">Total Amount</td>
                    ...
                    {invoice.status === "PAID" && (
                        <div className="bg-zinc-50 dark:bg-zinc-950/50 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className="flex-1 text-center md:text-left">
                                <h4 className="font-bold text-green-700 mb-1 leading-tight">Verified Payment</h4>
                                <p className="text-xs text-zinc-500">
                                    {transaction
                                        ? `Transaction ID: ${transaction.tranId} via ${transaction.cardType}`
                                        : "Successfully processed through our secure payment gateway."}
                                </p>
                            </div>
                            <div className="text-right font-mono text-[10px] text-zinc-400">
                                Paid On: {transaction ? new Date(transaction.tranDate).toLocaleString() : new Date().toLocaleString()}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="pt-12 border-t border-zinc-100 dark:border-zinc-800 text-center space-y-4">
                        <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">Thank you for being part of {settings?.madrasaName || "Internet Madrasa"}</p>
